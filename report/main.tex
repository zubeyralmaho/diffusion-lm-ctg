...
giving the rounding objective a meaningful geometry to start from. The
embeddings are then trained jointly with the rest of the network.
...
Table \ref{tab:main} reports the test-set comparison across the three
systems; Table \ref{tab:ablation} reports the Diffusion-LM ablations on
noise schedule, DDIM step count, and prefix length. Sections
\ref{sec:errors} and \ref{sec:discussion} interpret these in turn.
...
Our implementation supports both simultaneously. Along the diversity--
fluency axis the three systems are expected to occupy distinct points:
T5 beam search optimizes likelihood and collapses to templates, GPT-2
nucleus sampling trades a small BLEU drop for diversity, and Diffusion-LM
sits in between, with the rounding step pulling toward high-probability
tokens at every iteration.
...
generation on E2E NLG, augmented with self-conditioning, EMA, BERT-init,
and a jointly trained attribute classifier for gradient-based guidance
during DDIM sampling. The model combines hard prefix conditioning, which
enforces attribute control by construction, with soft classifier guidance,
which trades fluency for finer per-slot control, and was compared with
GPT-2 and T5 baselines along fluency, diversity, and controllability.
Three next steps follow from our limitations: a multi-head classifier
guiding all eight slots simultaneously, a parser-based MR-recovery metric
that credits paraphrase, and evaluation on a benchmark whose structure
rewards iterative refinement --- e.g.\ CommonGen, ROCStories --- where the
diffusion paradigm has more room to differentiate from beam search.
